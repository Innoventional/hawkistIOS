...
- (void)settingDate:(NSInteger)month year:(NSInteger)year
{
    ...
    self.setDate =[[NSCalendar currentCalendar] dateFromComponents:comps];
    
    self.datePath = [self datePath:self.setDate];
    
    [self selectRow: self.datePath.row
      inComponent: MONTH
         animated: NO];
...
{
    if (self.deleg && [self.deleg respondsToSelector: @selector(didSelect)])
        [self.deleg didSelect];
...
-(NSIndexPath *)todayPath // row - month ; section - year
{
    ...
    NSString *month = [self monthName:[NSDate date]];
    NSString *year  = [self yearName:[NSDate date]];
...
-(NSIndexPath*) datePath:(NSDate*)date
{
    CGFloat row = 0.f;
    CGFloat section = 0.f;
    
    NSString *month = [self monthName:self.setDate];
    NSString *year  = [self yearName:self.setDate];
